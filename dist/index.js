...
var _nil = _interopRequireDefault(__nccwpck_require3_(5350));
...
const _axios = /*#__PURE__*/ _interop_require_default(__nccwpck_require3_(8757));
const _fs = /*#__PURE__*/ _interop_require_default(__nccwpck_require3_(7147));
const _core = /*#__PURE__*/ _interop_require_default(__nccwpck_require3_(2186));
...
async function checkAuditStatus(auditTrackingIDs) {
    const res = await _axios.default.post('https://api.omnifractal.com/v1/checkAuditStatus', {
...
(async function() {
    try {
        const pages = _core.default.getInput('pages').split(/[\n\s]+/).map((page)=>page.trim());
        const budgetsPath = _core.default.getInput('budgetsPath');
        const budgets = budgetsPath ? JSON.parse(_fs.default.readFileSync(budgetsPath, 'utf8')) : undefined;
        const apiKey = _core.default.getInput('apiKey');
        // Retrieve additional information
        const branch = _core.default.getInput('branch');
        const repository = _core.default.getInput('repository');
        const waitForResults = _core.default.getInput('waitForResults') ? _core.default.getInput('waitForResults') === 'true' : true;
        const timeout = _core.default.getInput('timeout') ? parseInt(_core.default.getInput('timeout'), 10) : 300;
        if (timeout < 60) {
            _core.default.setFailed('Error: timeout must be at least 60 seconds');
            return;
        } else if (timeout > 600) {
            _core.default.setFailed('Error: timeout must be at most 600 seconds (10 minutes)');
            return;
        }
        const commit_sha = _core.default.getInput('commit_sha');
        const commit_message = _core.default.getInput('commit_message');
        const commit_author = _core.default.getInput('commit_author');
        const commit_author_email = _core.default.getInput('commit_author_email');
        let auditTrackingIDs = [];
        await _axios.default.post('https://api.omnifractal.com/v1/auditWithActions', {
            pages: pages,
            budgets: budgets,
            branch: branch,
            ...
            if (res.status >= 400) {
                _core.default.setFailed(`Error: ${res.status} - ${res.data.message || res.statusText || `Something went wrong`}`);
            } else {
                ...
                while(waitForResults){
                    ...
                    if (new Date().getTime() >= endTime) {
                        ...
                        _core.default.setFailed(message);
                        break;
                    }
                    ...
                    if (auditStatus.audits && auditStatus.audits.length > 0) {
                        ...
                        if (finishedAudits.length >= auditTrackingIDs.length) {
                            ...
                            _core.default.debug(message);
                            break;
                        }
                    } else {
                        _core.default.setFailed('Error: No response received from the server');
                        break;
                    }
            ...
            if (error.response) {
                ...
                _core.default.setFailed(`Error: ${error.response.status} - ${error.response.data.message || error.response.message || error.response.statusText || `Something went wrong`}`);
                ...
                _core.default.setFailed(`Error: ${error.message || error.data.message || `Something went wrong`}`);
                console.error('Error details:', error);
            }
        });
    } catch (error) {
        _core.default.setFailed(`Error: ${error.message}`);
        console.error('Error details:', error);
    }
})();

//# sourceMappingURL=data:application/json;base64,
})();
